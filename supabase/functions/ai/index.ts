...
//
// Модель можно поменять. Flash-Lite is enough for short kid-friendly tutor answers.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!text) {
      throw new Error(data?.promptFeedback?.blockReason ?? 'Gemini returned an empty answer.');
    }
